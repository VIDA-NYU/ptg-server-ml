...
from tim_reasoning import StateManager, BBNManager
...
class ReasoningApp:
    ...
    def __init__(self):
        ...
        self.bbn_manager = BBNManager(CONFIGS)

    def start_recipe(self, recipe_id):
        logger.info(f'Starting recipe, ID={str(recipe_id)}')
        if recipe_id is not None and recipe_id != 'tourniquet':
            ...
            while True:
                for sid, timestamp, data in await ws_pull.recv_data():
                    if recipe_id != 'tourniquet':
                        ...
                        if sid == RECIPE_SID:  # A call to start a new recipe
                            recipe_id = data.decode('utf-8')
                            first_step = self.bbn_manager.start_recipe(recipe_id)
                            if first_step is not None:
                                await ws_push.send_data([orjson.dumps(first_step)], re_check_status_sid)
                            continue
                        elif sid == UPDATE_STEP_SID:  # A call to update the step
                            step_index = int(data)
                            updated_step = self.bbn_manager.set_user_feedback(step_index)
                            if updated_step is not None:
                                await ws_push.send_data([orjson.dumps(updated_step)], re_check_status_sid)
                            continue
                        ...
                        if detected_objects is not None and detected_actions is not None and detected_steps is not None and counter >= 30:
